...
public class KombitStsUtilityTests
{
    ...
    public async Task GetRequestFromStsShouldReturnAssertion()
    {
        ...
        var response = await (await KombitStsClient.GetAssertion(stsUri, request)).Content.ReadAsStringAsync();
        await File.WriteAllTextAsync("StsResponse.xml", response);
        var responseXml = XDocument.Parse(response);
        Should.NotThrow(() => VerifySignature(responseXml).IfLeft(ex => throw ex));
